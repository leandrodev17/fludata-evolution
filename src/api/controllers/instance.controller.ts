...
export class InstanceController {
  ...
  public async createInstance(instanceData: InstanceDto) {
    try {
      ...
      await this.waMonitor.saveInstance({
        ...
        transcribeAudio: instanceData.transcribeAudio,
      });
  ...
  public async setTranscription({ instanceName }: InstanceDto, data: { transcribeAudio: boolean }) {
    try {
      const instance = await this.prismaRepository.instance.update({
        where: { name: instanceName },
        data: { transcribeAudio: data.transcribeAudio },
      });

      // Atualiza a instância em memória
      if (this.waMonitor.waInstances[instanceName]) {
        this.waMonitor.waInstances[instanceName].instance.transcribeAudio = data.transcribeAudio;
      }
      ...
      return {
        ...
        error: false,
        response: {
          transcribeAudio: instance.transcribeAudio,
          message: `Audio transcription ${data.transcribeAudio ? 'enabled' : 'disabled'}`,
        },
      };
    } catch (error) {
      throw new InternalServerErrorException(error.toString());
