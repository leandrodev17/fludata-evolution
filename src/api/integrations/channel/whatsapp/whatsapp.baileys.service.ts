...
export class BaileysStartupService extends ChannelStartupService {
    ...
    'messages.upsert': async (
      ...
      try {
        for (const received of messages) {
          ...
          this.logger.verbose(`DEBUG Gemini (recebido): transcribeAudio=${this.instance.transcribeAudio}, hasAudio=${!!received?.message?.audioMessage}`);
          if (this.instance.transcribeAudio && received?.message?.audioMessage) {
            this.logger.verbose('DEBUG Gemini: Iniciando transcrição de áudio recebido');
            try {
              ...
              this.logger.verbose(`DEBUG Gemini: Buffer baixado, tamanho: ${buffer.length}`);
              const audioBase64 = buffer.toString('base64');
              const { GeminiTranscriptionService } = await import('@api/services/gemini-transcription.service');
              const geminiService = new GeminiTranscriptionService();

              this.logger.verbose(`DEBUG Gemini: Serviço habilitado: ${geminiService.isEnabled()}`);
              if (geminiService.isEnabled()) {
                this.logger.verbose('DEBUG Gemini: Chamando API de transcrição...');
                const transcription = await geminiService.transcribeAudio(audioBase64);
                if (transcription) {
                  messageRaw.message.geminiTranscription = transcription;
                  this.logger.verbose(`Áudio transcrito via Gemini: ${transcription.substring(0, 50)}...`);
                } else {
                  this.logger.warn('DEBUG Gemini: Transcrição retornou null');
                }
              }
            } catch (error) {
              this.logger.error(`Erro ao transcrever áudio via Gemini: ${error.message}`);
            }
          }
  ...
  private async sendMessageWithTyping<T = proto.IMessage>(
    ...
    try {
      ...
      // Transcrição de áudio via Gemini (mensagens enviadas)
      this.logger.verbose(`DEBUG Gemini: transcribeAudio=${this.instance.transcribeAudio}, hasAudio=${!!messageSent?.message?.audioMessage}`);
      if (this.instance.transcribeAudio && messageSent?.message?.audioMessage) {
        this.logger.verbose('DEBUG Gemini: Iniciando transcrição de áudio enviado');
        try {
          const buffer = await downloadMediaMessage(
